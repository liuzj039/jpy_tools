...
def getScriptContent(snakemake, inline, contents, name, cluster):
    if inline:
        ...
        if snakemake:
            ...
            replaceContent = findContent + f" && jpy_sendMessage.py --title \'{cluster}上的☆{name}☆似乎出现了意外情况\' --content \'看不到我看不到我看不到我\'"
            contents = re.sub(r'(touch[\s\S]+?jobfailed)', replaceContent,
                              contents)
    if snakemake:
        suffixNoti = ""
    else:
...
def getServerScriptContent(cluster, node, time, queue, name, mem, threads,
                           gpu):
...
def parsePbsJobStatus():
    try:
        jobStatus = str(sh.grep(sh.qstat('-f', u='liuzj'), 'Job Id:', A=1))
    except:
        jobStatus = ''
    IdMap = {}
    try:
...
def main(contents, cluster, inline, snakemake, time, queue, name, mem, threads,
    ...
    differJobStatus = {
        x: y
        for x, y in afterJobStatus.items() if x not in beforeJobStatus.keys()
    }
