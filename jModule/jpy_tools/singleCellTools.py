...
def mergeAdataExpress(adata, groupby='louvain', useRaw=True, logTransformed=True, method='mean'):
    ...
    groupbyExpressDf = adataExpressDf.join(adata.obs[groupby]).groupby(groupby).agg(method)
...
def cellTypeAnnoByMarker(adata, allMarkerUse, label="louvain", method="mean"):
    ...
    finalMtx = (
        ...
        .rename({0: "express", 1: "ratio"}, axis=1)
        .reset_index()
    ).query("cellType != 'Others'")
    
    return finalMtx

def cellTypeAnnoByMarkerOld(adata, allMarkerUse, expressionMtx, zscoreby = 'cluster', method='mean'):
    """
    通过marker基因表达量鉴定细胞类型
...
def getAdataFromKbNucleiResult(t2gPath, ecPath, splicePath, unsplicePath, needUmiMappingInfo=False, adataPath=False):
    """
    ...
    kbMtxDf = kbDf.groupby('barcode')['geneLs'].apply(lambda df:df.value_counts()).unstack().fillna(0)
    kbSplicedMtxDf = spliceBusDf.query("barcodeUmi in @unoverlapUmiLs").groupby("barcode")["geneLs"].apply(
        lambda df: df.value_counts()
    ).unstack().reindex(kbMtxDf.index).reindex(kbMtxDf.columns, axis=1).fillna(0)
    kbUnsplicedMtxDf = unspliceBusDf.query("barcodeUmi in @unoverlapUmiLs").groupby("barcode")["geneLs"].apply(
        lambda df: df.value_counts()
    ).unstack().reindex(kbMtxDf.index).reindex(kbMtxDf.columns, axis=1).fillna(0)

    kbAd = creatAnndataFromDf(kbMtxDf, spliced=kbSplicedMtxDf, unspliced=kbUnsplicedMtxDf)
...
def extractReadCountsByUmiFromTenX(molInfoPath, filtered=True):
    ...
    allUmiCount = pd.DataFrame(np.c_[molInfo['umi'][()], \
