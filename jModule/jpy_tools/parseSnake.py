...
class SnakeHeader:
    ...
    Snakemake config should include two features:
        'pipelineDir',
        'resultDir' ,
    if using more than one sample,
        'sampleList' 
    also should provide
    """
    def __init__(self, snakeFile, configPath):
...
class SnakeRule:
    ...
    node : discarded
    """
    def __init__(self, snakeFile, name, step, threads=1, gpu=0):
        self.annotation = ''
        self.name = name
        self.step = str(step)
                ...
                for stepDir, singlePaths in singleList.items():
                    for singlePath in singlePaths:
                        if singlePath.endswith('/'):
                            ...
                            singleNameLs[1:] = [x.capitalize() for x in singleNameLs][1:]
